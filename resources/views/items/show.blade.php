            ...
            <div class="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="p-6">
                            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div class="rounded-xl bg-gray-50 border border-gray-100 p-4">
                                    ...
                                    <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $item->categoria ?: '—' }}</dd>
                                </div>
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
                        <div class="flex items-center justify-between px-6 py-4 border-b">
                            <div>
                                <h2 class="text-sm font-semibold text-gray-900">Movimientos</h2>
                                <p class="text-xs text-gray-500 mt-1">Bitácora del historial (trazabilidad).</p>
                            </div>
                        </div>
                                            ...
                                            <div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                                                <div class="text-sm font-semibold text-gray-900">
                                                    ...
                                                    <span class="text-gray-700">
                                                        {{ $m->estado_anterior ? ($m->estado_anterior.' → ') : '→ ' }}
                                                        {{ $m->estado_nuevo ?? '—' }}
                                                    </span>
                                                </div>
                                            ...
                                            <div class="mt-2 grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs text-gray-600">
                                                <div class="rounded-lg bg-gray-50 border border-gray-100 px-3 py-2">
                                                    ...
                                                    <span class="font-medium text-gray-800">
                                                        {{ $m->ubicacionAnterior?->nombre ? ($m->ubicacionAnterior->nombre.' → ') : '→ ' }}
                                                        {{ $m->ubicacionNueva?->nombre ?? '—' }}
                                                    </span>
                                                </div>
                                                ...
                                                <div class="rounded-lg bg-gray-50 border border-gray-100 px-3 py-2">
                                                    ...
                                                    <span class="font-medium text-gray-800">{{ $m->detalle ?? '—' }}</span>
                                                </div>
                                            </div>
                                        </li>
                                    @endforeach
                ...
                <div class="space-y-6">
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
                        ...
                        <div class="p-6 space-y-4">
                            {{-- Cambiar estado --}}
                            <form method="POST" action="{{ route('items.changeEstado', $item->id) }}" class="space-y-2">
                                @csrf
                                ...
                                <input name="detalle" placeholder="Detalle (opcional)"
                            ...
                            {{-- Mover ubicación --}}
                            <form method="POST" action="{{ route('items.moveUbicacion', $item->id) }}" class="space-y-2">
                                @csrf
